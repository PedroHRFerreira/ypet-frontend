	...
	setup() {
		...
		const optionsUFEnum = computed(() => {
			return UFEnum.value.map((item) => {
				return { ...(item as any), state: (item as any).state ?? "default" };
			});
		});
		...
		onMounted(async () => {
			...
			useCitizenEdit.setFormField(
				"can_report_abuse",
				citizen?.can_report_abuse || "",
			);
			useCitizenEdit.setFormField(
				"can_mobile_castration",
				citizen?.can_mobile_castration || "",
			);
		...
		async function onInputZipCode(value: string) {
			...
			if (digits.length === 8) {
				...
				if (useCitizenEdit.form.zip_code.value !== digits) return;
				if (data) {
					...
					useCitizenEdit.setFormField("complement", data.complement || "");
					const ufOpt = UFEnum.value.find(
						(o) =>
					...
					if (ufOpt) {
						...
						useCitizenEdit.setFormField("state", data.state);
					}
				}
			}
		}
