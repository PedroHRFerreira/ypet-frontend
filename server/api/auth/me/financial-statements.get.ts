export default defineEventHandler(async (event): Promise<IResponse> => {
	try {
		...
		const url = `${apiBaseUrl}/auth/me/financial-statements?${params.toString()}`;
		...
		return {
			...
			show: false,
			status: 200,
			message: "Meus extratos carregados com sucesso",
			data: response,
		} as IResponse;
	} catch (err) {
		...
		return {
			...
			message: error.message || "Erro ao carregar meus extratos",
			data: error,
		} as IResponse;
	}
});
